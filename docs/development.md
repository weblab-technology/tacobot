...
| --- | --- | --- |
| Unit | `tests/unit/` | Pure functions: `parser`, `format`, `give.validate`, `give.decide`, `receiver.verify`. Fast, no DB. |
| Integration | `tests/integration/` | Real handlers + real DB (PGlite): constraints, `users` upsert/ensure, `executeGive`, redemption, reaction give, cron reset, leaderboard command. |
